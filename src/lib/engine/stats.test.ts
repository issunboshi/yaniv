...
function makeGame(overrides: Partial<Game> = {}): Game {
  return {
    ...
    createdBy: 'kp1',
    players: [
      { playerId: 'kp1', name: 'Alice', avatar: '🃏', color: '#e74c3c', eliminated: false, displayOrder: 0 },
      { playerId: 'kp2', name: 'Bob', avatar: '🂡', color: '#3498db', eliminated: true, eliminatedAtRound: 3, displayOrder: 1 },
    ],
    rounds: [
      {
        id: 'r1', roundNumber: 1, handValues: { kp1: 5, kp2: 12 }, appliedScores: { kp1: 0, kp2: 12 },
        yanivCallerId: 'kp1', wasAssafed: false, assafPlayerIds: [], halvingEvents: [], eliminations: [],
        createdAt: '2026-01-01T00:00:00Z',
      },
      {
        id: 'r2', roundNumber: 2, handValues: { kp1: 7, kp2: 3 }, appliedScores: { kp1: 7, kp2: 0 },
        yanivCallerId: 'kp2', wasAssafed: false, assafPlayerIds: [], halvingEvents: [], eliminations: [],
        createdAt: '2026-01-01T00:01:00Z',
      },
      {
        id: 'r3', roundNumber: 3, handValues: { kp1: 4, kp2: 6 }, appliedScores: { kp1: 0, kp2: 206 },
...
describe('derivePlayerStats', () => {
  it('calculates wins and games played', () => {
    ...
    const alice = stats.find(s => s.playerId === 'kp1')!;
    expect(alice.gamesPlayed).toBe(1);
    expect(alice.wins).toBe(1);
    const bob = stats.find(s => s.playerId === 'kp2')!;
