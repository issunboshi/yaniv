<script lang="ts">
  ...
  async function doDelete(game: Game) {
    try {
      ...
      if (expandedId === game.id) expandedId = null;
    } catch (e) {
...
<div class="mx-auto max-w-lg px-4 py-6 space-y-4">
  ...
  {#if loading}
    <div class="py-16 text-center text-emerald-500 text-sm">Loading games...</div>
  {:else}
    <!-- Game count -->
          ...
          <div class="border-t border-emerald-800/40 px-4 py-3">
            ...
            <div class="mt-3 flex justify-between items-center">
              {#if game.status === 'in_progress'}
                <a href="/game/{game.code}" class="text-xs rounded px-3 py-1.5 bg-amber-500/20 text-amber-400 border border-amber-600/50 hover:bg-amber-500/30 transition-colors">
                  Resume
                </a>
              {:else}
                <div></div>
