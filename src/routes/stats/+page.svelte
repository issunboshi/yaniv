<script lang="ts">
  ...
  const playerStats = $derived(derivePlayerStats(games));
  const globalStats = $derived(deriveGlobalStats(games));
...
<div class="mx-auto max-w-lg px-4 py-6 space-y-6">
  ...
  {#if loading}
    <div class="py-16 text-center text-emerald-500 text-sm">Loading statistics...</div>
  {:else}
